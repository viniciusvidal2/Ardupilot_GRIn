...
const AP_Scheduler::Task Rover::scheduler_tasks[] = {
    ...
    SCHED_TASK(ahrs_update,            50,   7400),
...
void Rover::ahrs_update()
{
    ...
    // Envio de dados de gps e orientacao
    enviando_dados_relevantes();

    // Obtendo orientacoes corretas - VINICIUS
    // ---------------------------------------
    obter_bearing_correto();
}
...
void Rover::obter_bearing_correto(void)
{
    ...
    if((uint16_t)gps.ground_speed_cm() > (uint16_t)g.vel_min_gps && ahrs.have_gps_porra() && gps.status() >= AP_GPS::GPS_OK_FIX_3D){
