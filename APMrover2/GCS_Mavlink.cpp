...
void Rover::send_nav_controller_output(mavlink_channel_t chan)
{
    mavlink_msg_nav_controller_output_send(
        ...
        angulo_pitch_altura, //ahrs.pitch_sensor * 0.01f, //ahrs.groundspeed() * ins.get_gyro().z,  // use nav_pitch to hold actual Y accel
        angulo_atual, //nav_controller->nav_bearing_cd() * 0.01f,
        next_navigation_leg_cd * 0.01f, //nav_controller->target_bearing_cd() * 0.01f,
        distancia_controlada, //wp_distance,
        0,
        mission.num_commands(), //groundspeed_error,
        indice_wp_buscado); //nav_controller->crosstrack_error());
}
...
GCS_MAVLINK_Rover::data_stream_send(void)
{
    ...
    if (stream_trigger(STREAM_EXTENDED_STATUS)) {
        ...
        //send_message(MSG_NAV_CONTROLLER_OUTPUT); // Enviando forcado da rotina principal
    }
